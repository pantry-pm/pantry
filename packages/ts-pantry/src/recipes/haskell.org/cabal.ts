...
export const recipe: Recipe = {
  ...
  build: {
    script: [
      ...
      '  fi',
      'done',
      '# 42 means "upstream has nothing for this version", not "the build broke".',
      'if [ -z "$ASSET" ]; then echo "no cabal asset for {{hw.platform}}/{{hw.arch}} at {{version}} (tried: $SUFFIXES)" >&2; exit 42; fi',
