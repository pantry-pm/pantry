...
export const recipe: Recipe = {
  ...
  // Resolve the version from Cursor's own official download API (no Homebrew).
  // The same endpoint feeds the build's download URL below.
  versionSource: {
    type: 'custom',
    fetch: async (): Promise<string[]> => {
      const res = await fetch('https://www.cursor.com/api/download?platform=darwin-arm64&releaseTrack=stable', { redirect: 'follow' })
  ...
  build: {
    script: [
      ...
      '# Resolve the real download URL from Cursor\'s official API (the SHA-pinned',
      '# downloads.cursor.com path can\'t be templated from the version alone).',
