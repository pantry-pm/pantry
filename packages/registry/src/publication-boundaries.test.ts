...
describe('first-party publication boundaries', () => {
  ...
  it('bounds retained-artifact backfill scans and closes every HTTP connection', () => {
    ...
    expect(workflow).toContain('DOMAIN_SHARD_COUNT=64')
    expect(workflow).toContain('ARTIFACT_SHARD_COUNT=4')
    expect(workflow).toContain('--artifact-shard "${SHARDS#*:}"')
    expect(workflow).toContain('max-parallel: 8')
    expect(workflow.match(/PANTRY_BACKFILL_SCAN_CONCURRENCY: '2'/g)).toHaveLength(2)
    ...
    expect(backfill).toContain('maxBytes: 2 * 1024 * 1024 * 1024')
