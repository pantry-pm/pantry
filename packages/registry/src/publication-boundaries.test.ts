...
describe('first-party publication boundaries', () => {
  ...
  it('deploys the exact bundle only after scanner readiness is proven', () => {
    ...
    expect(workflow).toContain('set_env PANTRY_SCANNER_WORKER_MEMORY_HIGH 512M')
    expect(workflow).toContain('set_env PANTRY_SCANNER_WORKER_MEMORY_MAX 768M')
  ...
  it('keeps CLI provisioning within the registry host resource budget', () => {
    ...
    expect(setup).toContain('set_env PANTRY_SCANNER_WORKER_MEMORY_HIGH 512M')
    expect(setup).toContain('set_env PANTRY_SCANNER_WORKER_MEMORY_MAX 768M')
