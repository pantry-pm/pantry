...
**No unsupervised macOS runs — macOS builds are supervised (`workflow_dispatch`) only.** macOS runners bill ~10×, so **no workflow may spawn a macOS runner on a `schedule:` or `push:` trigger.** The `schedule:` cron has been removed from every Mac-spawning workflow — `build.yml`, `build-versions.yml`, `build-residual.yml`, `sync-binaries.yml`, and `build-orchestrator.yml` (also disabled in the Actions tab). They keep `workflow_dispatch` so you can run a **supervised** macOS build by hand. Do not add a `schedule:`/`push:` trigger to any workflow whose matrix includes `macos-*`; if you need a new Mac matrix leg, gate it behind `workflow_dispatch` only. darwin-arm64 coverage that does NOT need a Mac stays automated on the cheap ubuntu runners:

- **darwin-arm64 DOWNLOAD recipes** → `mirror.yml` (every 6h, download-only, all platforms) + the Hetzner xdl fleet (continuous). No Mac.
- **new versions on update** → `publish-changed-packages.yml`, which is now **ubuntu-only and download-first**: `linux-x86-64` + `linux-arm64` run `--pkgx-mirror` (mirror pkgx's prebuilt; source-build only what pkgx lacks or we customize — php/postgres — natively), and `darwin-arm64` runs FROM UBUNTU with `--pkgx-mirror --mirror-only` (download the pkgx darwin prebuilt; never source-build on a miss, since darwin can't cross-compile on linux). Its old `macos-14` matrix leg was removed. Apps/fonts are excluded here (handled by check-desktop-updates).
- **desktop apps + fonts** → `check-desktop-updates.yml` (daily, ubuntu, via the darling-dmg/apfs-fuse/7zz hdiutil shim).
- **darwin-arm64 SOURCE recipes** (CLI tools pkgx doesn't prebuild, custom builds) are the ONLY thing that still needs a Mac — build those with a supervised `Build` (`build.yml`) `workflow_dispatch`.
